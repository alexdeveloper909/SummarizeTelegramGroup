...
from telegram_group_summarizer.summary_input import bundle_to_json, bundle_to_markdown, write_summary_bundle
...
def main() -> None:
    ...
    parser.add_argument("--output", help="Optional output path. For --format both, this is used as the file prefix.")
    ...
    if args.output:
        print(json.dumps({key: str(value) if value else None for key, value in output_paths.items()}, sort_keys=True))
        return
    ...
    if args.format == "json":
        ...
        print(json.dumps({"json": bundle_to_json(bundle), "markdown": bundle_to_markdown(bundle)}, sort_keys=True))
